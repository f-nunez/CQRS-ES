...
public class MongoDbEventStore<T> : IEventStore<T> where T : IAggregateRoot
{
    private readonly IMongoDbRepository _repository;
    private readonly IServiceBus _serviceBus;

    public MongoDbEventStore(IMongoDbRepository repository, IServiceBus serviceBus)
    {
    ...
    public async Task<T?> ReadStreamEventsAsync<TId>(TId id)
    {
        ...
        var storedEvents = events.OrderBy(x => x.Version).Select(Deserialze);
    ...
    public static BaseEvent Deserialze(StreamState @event)
    {
        BaseEvent? data;
    ...
    public static StreamState MapStreamState(BaseEvent @event, string streamName, long version)
    {
